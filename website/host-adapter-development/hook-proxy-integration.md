...
From [HOSTS.md](https://github.com/zjunlp/LightRSI/blob/main/components/adapters/HOSTS.md): uses Codex config mutation, hook registration, and a local OpenAI-compatible Responses proxy. Preserves the current active Codex provider name and reroutes that provider's `base_url` through the local proxy. Hooks registered: `SessionStart`, `PreToolUse`, and `PostToolUse`. Legacy `Stop` hooks are removed during reinstall.
