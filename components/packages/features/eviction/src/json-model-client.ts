      ...
      try {
        try {
          ...
          if (!response.ok) {
            ...
            const fallbackAllowed = response.status === 404
              || response.status === 405
              || response.status >= 500;
            if (!fallbackAllowed) {
              throw new Error(`responses_api_failed:${response.status}:${errorText}`);
            }
            throw new Error(`responses_api_fallback:${response.status}:${errorText}`);
          }
          const payload = await response.json();
          return { text: extractResponsesText(payload), usage: extractUsage(payload) };
        } catch (error) {
          const message = error instanceof Error ? error.message : String(error);
          if (!message.includes("responses_api_fallback:")) throw error;
        }
