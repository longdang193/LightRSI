...
test("does not fall back after authentication failure", async () => {
  const originalFetch = globalThis.fetch;
  let calls = 0;
  globalThis.fetch = async () => {
    calls += 1;
    return { ok: false, status: 401, async text() { return "unauthorized"; } } as Response;
  };
  try {
    const client = createApiJsonModelClient({
      baseUrl: "https://example.test/v1", apiKey: "secret", model: "model-a",
