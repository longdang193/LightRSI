...
test("applied receipt records committed token savings instead of scheduled estimate", () => {
  const prepared = execution();
  const result = buildCodexCleanerAppliedReceipt({ execution: prepared, epoch: epoch() });
