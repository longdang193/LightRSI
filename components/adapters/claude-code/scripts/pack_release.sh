...
cd "${REPO_ROOT}"
pnpm --filter @lightrsi/claude-code-adapter build >/dev/null
pnpm --filter @lightrsi/cli build >/dev/null
pnpm --filter @lightrsi/mcp build >/dev/null

PACK_TMP_DIR="$(mktemp -d /tmp/lightrsi-claude-code-pack-XXXXXX)"
cleanup() {
  rm -rf "${PACK_TMP_DIR}"
}
trap cleanup EXIT
...
NPM_CACHE_DIR="${NPM_CACHE_DIR:-/tmp/lightrsi-npm-cache}"
